...
public class DBConnection {
	...
	public DBConnection() {
		try {
			...
			throw new IllegalStateException("Unable to find data source named '" + JNDI_DS_NAME + "'", e);
		}
	}
	...
	public void storeMeasurement(LocalDateTime time, double value, String sensor) {
		withConnection(connection -> {
			PreparedStatement s = connection
					.prepareStatement("INSERT INTO log (datetime, temperature, sensor) VALUES (?,?,?)");
	...
	public List<Sensor> getSensors() {
		...
		withConnection(connection -> {
			PreparedStatement s = connection.prepareStatement("SELECT id,location FROM sensor");
			ResultSet rs = s.executeQuery();
			while (rs.next()) {
				sensors.add(new Sensor(rs.getString("id"), rs.getString("location")));
			}
		});
		return sensors;
	...
	public List<Measurement> getAllMeasurements(String sensorId, LocalDateTime after) {
		List<Measurement> result = new ArrayList<>();
		withConnection(connection -> {
			PreparedStatement s = connection
					.prepareStatement("SELECT datetime,temperature,sensor FROM log where datetime > ? AND sensor = ?");
			...
			while (rs.next()) {
				...
				Measurement measurement = new Measurement(time, rs.getDouble("temperature"));
